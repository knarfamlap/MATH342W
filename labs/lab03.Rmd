...
linear_svm_learning_algorithm = function(Xinput, y_binary, MAX_ITER = 5000, lambda = 0.1){
  ...
  hinge_err = function(x, y, w, b) {
    condition =  (1/2) - (y - (1/2)) * (w * x - b)
    sum(max(0, condition))
  }
  
  norm_vec <- function(x) sqrt(sum(x^2))
  
  cost = function(n, h_err, lambda, w) {
    (1 / n) * h_err + lambda * norm_vec(w)
  }
  ...
  for (inter in 1:MAX_ITER){
    for (i in 1:n) {
      condition = y_binary[i] * ((Xinput[i, ] %*% w) - b)
      if (condition >= 1) {
        w = w - (lr * (2 * lambda * w))
      } else {
        w = w -  (lr * (2 * lambda * w - (Xinput[i, ] * y_binary[i])))
        b = b - (lr * y_binary[i])
      }
    }
  }
