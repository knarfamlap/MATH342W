...
The absolute angle difference from ninety is converging to zero. In a high
dim space, random directions are orthogonal. 
...
It fails because X is not rank defficient. t(X) has no inverse
...
t(X) is rank defficient so we can solve for it
...
Its expected to be different than 1  
...
There are many orthonomal basis.
